...
let originalScene;
let originalTransitionDuration;
...
function initializeSettings() {
    ...
    if (settings.has('scene')) {
        secretsScene = settings.get("scene");
    }
    if (settings.has('autoSwitchBack')) {
        autoSwitchBack = settings.get("autoSwitchBack");
    }
}
...
function activate(context) {
    ...
    let workspaceSettingsChangedEvent = vscode.workspace.onDidChangeConfiguration(changes => {
        if (!changes.affectsConfiguration("obs.secretsSwitchScene")) {
            return;
        }
        initializeSettings();
    });
